...
#leitura do arquivo .pkl criado em metricas.py
...
#criando boxes de seleção de redes e cursos
...
#criando as colunas e colocando o que será selecionável em cada uma
...
#criação do data frame com todas as colunas depois do dummies
...
#junção das variáveis dummies com as variáveis numéricas
df = pd.concat([df_template, df]).fillna(0)


#modelo de probabilidade de felicidade utilizando o arquivo criado em metricas.py
proba = model['model'].predict_proba(df[model['features']])[:,1][0]


#if para aparecer no quadro a porcentagem com a cor correspondente
if proba > 0.7:
